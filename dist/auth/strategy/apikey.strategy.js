...
let ApiKeyStrategy = class ApiKeyStrategy extends (0, passport_1.PassportStrategy)(passport_headerapikey_1.HeaderAPIKeyStrategy, "api_key") {
    constructor(authService) {
        super({ header: "api_key", prefix: "" }, true, async (apiKey, done) => {
            if (this.authService.verifyApiKey(apiKey)) {
                done(null, true);
            }
            done(new common_1.UnauthorizedException("Invalid api key"), null);
        });
